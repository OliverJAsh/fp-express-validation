// https://github.com/gcanti/fp-ts/blob/260ab3a720bed237fef2d2c18761f71c3a1523b7/examples/TaskEither.ts#L7
...
export class TaskEither<L, A> {
    ...
    fold<R>(eitherLeft: (l: L) => R, eitherRight: (a: A) => R): task.Task<R> {
        return eitherT.fold(task)(eitherLeft, eitherRight, this.value);
    }
...
export function fromPromise<L, A>(
    f: () => Promise<A>,
    onrejected: (reason: any) => L,
): TaskEither<L, A> {
...
declare module 'fp-ts/lib/Chain' {
    interface Ops {
        flatten(
            chain: Chain<URI>,
        ): <L, A>(mma: TaskEither<L, TaskEither<L, A>>) => TaskEither<L, A>;
    }
}
